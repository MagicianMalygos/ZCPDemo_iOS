...
+ (NSArray *)zcp_backtraceOfMainThread {
    thread_t thread = bs_machThreadFromNSThread([NSThread mainThread]);
    ...
    _STRUCT_MCONTEXT machineContext;
    if(!bs_fillThreadStateIntoMachineContext(thread, &machineContext)) {
        NSLog(@"Fail to get information about thread: %u", thread);
        return nil;
    }
    ...
    NSMutableArray *arr = [NSMutableArray arrayWithCapacity:backtraceLength];
    
    for (int i = 0; i < backtraceLength; ++i) {
        const int entryNum = i;
        const uintptr_t address = backtraceBuffer[i];
        const Dl_info* const dlInfo = &symbolicated[i];
        ...
        NSString *nsAddress = [NSString stringWithFormat:@"0x%08lx", (uintptr_t)address];
        NSString *nsInfo = [NSString stringWithFormat:@"%-30s  0x%08" PRIxPTR " %s + %lu\n" ,fname, (uintptr_t)address, sname, offset];
        [arr addObject:@{@"addr": nsAddress, @"info": nsInfo}];
    }
    return arr;
}
