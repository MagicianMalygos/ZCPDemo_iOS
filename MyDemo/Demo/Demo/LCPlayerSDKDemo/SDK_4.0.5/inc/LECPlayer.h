...
//切换码率接口
- (BOOL) switchSelectStreamRateItem:(LCStreamRateItem *) selectStreamRateItem;
//检测某操作是否可以进行
...
//async
- (BOOL) switchSelectStreamRateItem:(LCStreamRateItem *) selectStreamRateItem completion:(void (^)())completion;
//async
...
@property (nonatomic, readonly) LCStreamRateItem *selectedStreamRateItem;//该变量标示当前使用码率，该变量只读，需要切换码率请调用switchSelectStreamRateItem方法实现
