...
def main():
    ...
    if len(current_repo_current_branchs) > 0:
        ...
        print('正在拉取当前分支，当前分支为：' + current_branch)
...
#     os.chdir(current_path)
#     os.system('sh clear_cocopods.sh')
...
def func_clone_hellotrip(path_go_back,current_path,manifest_branch):
    ...
    if 'HelloTrip_iOS_New' in current_path:
        os.chdir(current_path)
        #os.system('git checkout ' + manifest_branch)
        #os.system('git pull origin ' + manifest_branch)
    ...
    #当前目录中需要保留
    files_list = ['Pods','pod_update1.6.1.py', 'pod_update1.4.py']
    func_copy_files(hellotrip_replace_path,current_path,files_list)
    os.chdir(current_path)

def func_copy_files(hellotrip_replace_path, current_path, files_list):
    current_dirs_list = os.listdir(current_path)
    for current_f in current_dirs_list:
        current_file_path = os.path.join(current_path,current_f)

        if os.path.isdir(current_file_path) and current_f not in files_list:
            shutil.rmtree(current_file_path, True)


        elif os.path.isfile(current_file_path) and current_f not in files_list:
            os.remove(current_file_path)

    dirs_list = os.listdir(hellotrip_replace_path)

    for f in dirs_list:

        file_path = os.path.join(hellotrip_replace_path,f)
        current_file_path = os.path.join(current_path,f)

        if os.path.isdir(file_path) and f not in files_list:
            shutil.copytree(file_path, current_file_path)


        elif os.path.isfile(file_path) and f not in files_list:
